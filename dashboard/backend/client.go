...
var ErrInvalidBearerToken = errors.New("dashboard bearer token is invalid")
...
// TokenAuthenticator verifies a Dashboard login credential and returns the
...
// TokenReviewAuthenticator validates Dashboard login credentials through the
// Kubernetes TokenReview API using the Dashboard ServiceAccount.
...
func (a *TokenReviewAuthenticator) Authenticate(ctx context.Context, token string) (string, error) {
	if a == nil || a.client == nil {
		return "", errors.New("dashboard TokenReview client is not configured")
	}
